      ...
      <div className="mb-6">
        ...
        <div className="flex flex-wrap gap-5 text-sm text-[#4f6368] mb-4">
          <span className="flex items-center gap-1.5">
            <Folder className="w-4 h-4" /> {session.directory}
          </span>
          <span className="flex items-center gap-1.5">
            <Clock className="w-4 h-4" /> {date}
          </span>
          <span className="flex items-center gap-1.5">
            <Hash className="w-4 h-4" /> {session.slug}
          </span>
        </div>
        <div className="flex flex-wrap gap-3 pt-4 border-t border-[#c9d8d5]">
          <Badge
            variant="secondary"
            className="bg-[#f4f9f7] hover:bg-[#e5f1ec] text-[#102124] border-[#c9d8d5] font-normal px-3 py-1 rounded-full text-xs"
          >
...
function MessageItem({ msg, formatTokens }: { msg: Message; formatTokens: (n: number) => string }) {
  ...
  const getAgentRoleHtml = () => {
    ...
    return (
      <>
        {agentIcon ? (
          <img src={agentIcon} alt={agentName} className="w-3.5 h-3.5 inline-block mr-1" />
        ) : (
          "🤖 "
        )}
        {agentName}
  ...
  return (
    <article
      className={`w-fit max-w-[85%] border border-[#c9d8d5] rounded-2xl p-4 shadow-sm ${
        isUser
    ...
    >
      <div className="flex items-center gap-2 flex-wrap mb-3">
        <span
          className={`inline-flex items-center h-6 px-2.5 rounded-full border text-xs font-semibold bg-white/80 ${
        ...
        {!isUser && msg.mode && (
          <span className="inline-flex items-center h-6 px-2.5 rounded-full border border-[#ffb74d] bg-[#fff3e0] text-[#e65100] text-xs">
            {ModelConfig.agents[msg.mode]?.icon ? (
              <img
                src={ModelConfig.agents[msg.mode].icon}
                alt={msg.mode}
        ...
        {!isUser && msg.model && (
          <span className="inline-flex items-center h-6 px-2.5 rounded-full border border-[#ce93d8] bg-[#f3e5f5] text-[#7b1fa2] text-xs">
            {ModelConfig.getProvider(msg.model)?.icon ? (
              <img
                src={ModelConfig.getProvider(msg.model)?.icon as string}
                alt={msg.model}
                className="w-3.5 h-3.5 mr-1"
              />
            ) : (
  ...
  return (
    <div className="bg-white rounded-lg border border-[#e0e0e0] overflow-hidden">
      ...
      >
        ...
        {status === "completed" && (
          <span className="text-[11px] px-2.5 py-0.5 rounded-full font-medium bg-[#e8f5e9] text-[#4caf50]">
            ✓ 完成
          </span>
        )}
        {status === "error" && (
          <span className="text-[11px] px-2.5 py-0.5 rounded-full font-medium bg-[#ffebee] text-[#f44336]">
            ✗ 错误
          </span>
        )}
        {status === "running" && (
          <span className="text-[11px] px-2.5 py-0.5 rounded-full font-medium bg-[#fff3e0] text-[#ff9800]">
            ⏳ 运行中
          </span>
        )}
      ...
      {expanded && (
        <div className="p-3 bg-white border-t border-[#e0e0e0]">
          <div className="mb-3">
            <div className="text-[11px] font-semibold text-[#7a8b8f] uppercase tracking-wider mb-2">
              输入
            </div>
