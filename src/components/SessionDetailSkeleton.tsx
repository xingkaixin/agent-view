const SKELETON_MESSAGES = [
  {
    ...
    roleWidth: "w-14",
    timeWidth: "w-20",
    bodyWidths: ["w-full", "w-10/12", "w-7/12"],
  },
  {
    id: "user-short-1",
    roleWidth: "w-16",
    timeWidth: "w-24",
    bodyWidths: ["w-9/12", "w-7/12"],
  },
  {
    id: "agent-long-2",
    roleWidth: "w-14",
  ...
  },
  {
    ...
    roleWidth: "w-16",
    timeWidth: "w-20",
    bodyWidths: ["w-10/12", "w-8/12"],
  },
  {
    id: "agent-long-3",
    roleWidth: "w-14",
    timeWidth: "w-24",
    bodyWidths: ["w-full", "w-10/12", "w-9/12"],
  },
];
...
export function SessionDetailSkeleton() {
  return (
    <div
      ...
      className="mx-auto flex min-h-full w-full max-w-5xl flex-col gap-8 px-2 md:px-4"
    >
      <div className="flex flex-1 flex-col gap-8">
        {SKELETON_MESSAGES.map((item) => (
          <article
            key={item.id}
            data-testid="session-skeleton-message"
              ...
              <div className="min-w-0 flex-1 space-y-3">
                ...
                <div className="rounded-sm border border-[var(--console-border)] bg-white p-4 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
                  <div className="space-y-2">
                    {item.bodyWidths.map((width) => (
                      <SkeletonBlock
                        key={`${item.id}-${width}`}
                        className={`${width} h-3 animate-pulse`}
                      />
                    ))}
                  </div>
                </div>
